...
export class UserRepository implements ReadUsersRepo {
    ...
    private inMemoryUsers: User[] = [
        { userId: "123", username: "Game Master", password: "aA1!", roleId: "Game master" },
        { userId: "456", username: "Player1", password: "player1", roleId: "player" },
        { userId: "789", username: "Player2", password: "player2", roleId: "player" }

    ]

    public findAllByUsername(username: string): User[] {
        return this.inMemoryUsers.filter((user) => user.username === username);
    }
}